...
void InitTetris(void){
	...
	for (int j = 0; j < HEIGHT; j++)
		for (int i = 0; i < WIDTH; i++)
			field[j][i] = 0, (recRoot->f)[j][i] = 0; // recRoot 초기화

	nextBlock[0] = rand() % 7;
	nextBlock[1] = rand() % 7;
	nextBlock[2] = rand() % 7;
	nextBlock[3] = rand() % 7; // nextBlock 배열 초기화
	blockRotate = 0;
	blockY = -1;
	blockX = WIDTH / 2 - 2; // Block의 초기 상태 선언
	score = 0;
	gameOver = 0;
	timed_out = 0;
	...
	PrintScore(score); // 게임 판 초기화
	modified_recommend(recRoot); // 첫번째 추천 계산
	return;
}
...
void BlockDown(int sig){
		...
		else if (blockY != -1)
		{
			...
			for (int i = 0; i < HEIGHT; i++)
				for (int j = 0; j < WIDTH; j++)
					(recRoot->f)[i][j] = field[i][j]; // nextBlock 갱신 시 recRoot 초기화
			modified_recommend(recRoot); // 다시 recRoot를 root로 하는 tree를 기반으로 추천 정보 계산
	...
	{ // 3번: AI가 플레이하는 경우
		if (CheckToMove(field, currentBlock, blockRotate, blockY + 1, blockX))
		{
			score += AddBlockToField(field, currentBlock, recommendR, recommendY, recommendX);
			score += DeleteLine(field); // 삽입할 차례인경우 추천 위치에 즉시 삽입
			PrintScore(score);
			nextBlock[0] = nextBlock[1];
			nextBlock[1] = nextBlock[2];
			nextBlock[2] = nextBlock[3];
			nextBlock[3] = rand() % 7;
			recRoot->lv = 0, recRoot->score = 0;
			memcpy(recRoot->f, field, sizeof(field)); // nextBlock 갱신 시 recRoot 초기화
			modified_recommend(recRoot); // 다시 recRoot를 root로 하는 tree를 기반으로 추천 정보 계산
			blockY = -1, blockX = WIDTH / 2 - 2, blockRotate = 0;
			DrawField(); // 낙하지점 반영한 필드 출력
			DrawNextBlock(nextBlock);
			DrawBlockWithFeatures(blockY + 1, blockX, nextBlock[0], blockRotate);
		}
		else
		{
			gameOver = 1;
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			...
			{ // 현재 그린 칸의 한 칸 밑에 이미 차있다면 다른 블록과 닿은 것, HEIGHT - 1이면 바닥과 닿은 것
				...
				f[blockY + i][blockX + j] = 1; // 필드에 칸 추가
			}
		}
	}
	return touched * 10;
...
void newRank(int score) {
	...
	if (score_number != 0) {
		...
		rankList = newNode; // rankList가 비어있는 경우
	score_number++;
	writeRankFile();
	return;
}

int maxHeight(char f[HEIGHT][WIDTH]) {
	int maxY = 0, flag = 0;
	for (maxY = 0; maxY < HEIGHT; maxY++)
	{ // 위에서부터 내려가며 블럭이 나오는 행에서 break
		for (int x = 0; x < WIDTH; x++)
			if (f[maxY][x] == 1) flag = 1;
		if (flag == 1) break;
	}
	return HEIGHT - maxY; // 블럭이 없으면 0이 반환될 것
}

int countRotation(int blockShape) {
	switch (blockShape) {
	case 0:
	case 1:
	case 2:
...
int countBlanks(char f[HEIGHT][WIDTH], int height) {
	int cnt = 0;
...
int modified_recommend(RecNode* root) {
	int max = -1, childIdx = 0; // 미리 보이는 블럭의 추천 배치까지 고려했을 때 얻을 수 있는 최대 점수
	int curBlockID = nextBlock[root->lv]; // 현재 고려하고 있는 BlockID
	int rotation = countRotation(curBlockID);  // 현재 블록의 최대 회전 수
	int startY = -1, startX, update = 0;
	for (int i = 0; i < rotation; i++)
	{ // 각 블록에 따른 회전에 대해서 계산
		for (int j = -BLOCK_WIDTH; j < WIDTH; j++)
		{ // 왼쪽부터 오른쪽까지 이동할 수 있는 x좌표에 대해 반복
			startY = -1, startX = j, update = 0;
			while (CheckToMove(root->f, curBlockID, i, startY + 1, j))
				startY++; // 내릴 수 있는 곳까지 내리기
			if (startY < 0) continue; // 만약 움직일 수 없으면 계산에서 제외
			RecNode* childNode = (RecNode*)malloc(sizeof(RecNode)); // 새로운 childNode 생성
			childNode->lv = root->lv + 1; // level 초기화
			memcpy(childNode->f, root->f, sizeof(childNode->f)); // childNode의 field를 root로부터 복사
			childNode->score = root->score; // 점수 초기화
			childNode->score += AddBlockToField(childNode->f, curBlockID, i, startY, startX);
			childNode->score += DeleteLine(childNode->f); // childNode->f를 기준으로 Block을 삽입했을 때 점수 변화 반영
			if (childNode->lv < VISIBLE_BLOCKS)
				childNode->score += modified_recommend(childNode); // childNode를 root로 다시 탐색 진행
			if (max < childNode->score)
				update = 1; // 점수가 높으면 무조건 갱신
			else if (max == childNode->score)
			{ // 점수가 같으면 2가지 경우로 분할
				int childHeight = maxHeight(childNode->f), rootHeight = maxHeight(root->f);
				int childBlanks = countBlanks(childNode->f, childHeight), rootBlanks = countBlanks(root->f, rootHeight);
				if (childHeight < rootHeight)
					update = 1; // 높이가 더 낮은 경우 우선 순위
				else if (childHeight == rootHeight && childBlanks < rootHeight)
					update = 1; // 높이도 같은 경우 빈 칸 개수가 더 적은 경우가 우선
			}
			if (update == 1)
			{ // 갱신 하는 경우 root 여부에 따라 추천 Block 정보도 갱신
				max = childNode->score; // max score 갱신
				if(root->lv == 0) // root인 경우에만 recommend 좌표 갱신
					recommendR = i, recommendY = startY, recommendX = j;
			}
			free(childNode);
		}
	}
