...
void InitTetris(void){
	...
	nextBlock[0] = rand() % 7;
	nextBlock[1] = rand() % 7;
	nextBlock[2] = rand() % 7; // nextBlock 배열 초기화
	blockRotate = 0;
	blockY = -1;
...
void BlockDown(int sig){
	...
	else if (blockY != -1)
	{
		...
		for (int i = 0; i < CHILDREN_MAX; i++)
			recRoot->c[i] = NULL;
		recommend(recRoot); // 다시 recRoot를 root로 하는 tree를 기반으로 추천 정보 계산
...
int recommend(RecNode* root){
	...
	int childIdx = 0, curBlockID = nextBlock[root->lv]; // root->c의 인덱스와 현재 고려하고 있는 BlockID
	for (int i = 0; i < 4; i++)
	{ // 각 회전에 대해서 계산 (지금은 Block이 회전하면 같아지는 경우는 고려 x)
		for (int j = -BLOCK_WIDTH; j < WIDTH; j++)
		{ // 왼쪽부터 오른쪽까지 이동할 수 있는 x좌표에 대해 반복
			int startY = -1, startX = j;
			...
			root->c[childIdx++] = childNode; // root->c에 childNode 연결
			if (childNode->lv < VISIBLE_BLOCKS)
				childNode->score += recommend(childNode); // childNode를 root로 다시 탐색 진행
			if (max < childNode->score)
			{ // max가 갱신된 경우 root 여부에 따라 추천 Block 정보도 갱신
